...
public class SchoolClassesController {
	...
	public String displayEditSchoolClassForm(@RequestParam(value="schoolClassId", required=false) String schoolClassId,
											 Model model, HttpSession session) {
		if (session.getAttribute("userLogin") == null)
			return "redirect:/Login";

		model.addAttribute("schoolClasses", DatabaseConnector.getInstance().getSchoolClass(schoolClassId));
		return "schoolClassEditForm";
	}

	@RequestMapping(value="/ApplyClassEdit", method=RequestMethod.POST)
	public String editSchoolClass(@RequestParam(value="profile", required=false) String profile,
		...
		schoolClass.setId(Integer.valueOf(schoolClassId));

		DatabaseConnector.getInstance().updateSchoolClass(schoolClass);

		model.addAttribute("schoolClasses", DatabaseConnector.getInstance().getSchoolClasses());
		model.addAttribute("message", "Klasa została edytowana");
